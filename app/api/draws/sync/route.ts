...
import { fetchDrawsHTML, parseDrawsFromHTML, validateDraw, generateDrawId } from '@/lib/scraper'
  ...
  try {
    ...
    if (lastSync && now - lastSync.timestamp < CACHE_DURATION) {
      return NextResponse.json(
        { error: 'Sync too recent. Please wait before syncing again.', nextSync: new Date(lastSync.timestamp + CACHE_DURATION) },
        { status: 429 }
      )
    }

    const url = 'https://www.reducmiz.com/resultat_fdj.php?jeu=loto&nb=2412'
    ...
    if (validDraws.length === 0) {
      return NextResponse.json(
        { error: 'No valid draws found in source', parsedCount: parsedDraws.length },
        { status: 500 }
      )
    }
    ...
    for (const draw of validDraws) {
      try {
        ...
        errors.push(`Error processing draw ${draw.dateISO}: ${error instanceof Error ? error.message : 'Unknown error'}`)
      }
    }
